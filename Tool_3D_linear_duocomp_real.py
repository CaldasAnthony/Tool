...
T_iso, P_surf, P_tau = 1500., 1.e+6, 1.e+3
...
lim_alt, rupt_alt, beta = h, 0.e+0, 0.*theta_step*180./np.pi
...
if Composition == False :
    save_name_3D = "%s%s_3D_duo_linear_real_%.2f_%.2f_%i_%i_%.2f"%(save_adress,name_exo,np.amin(x_ratio_species_active),np.amax(x_ratio_species_active),T_iso,beta,P_tau/(1.e+5))
else :
    save_name_3D = "%s%s_3D_duo_linear_real_%.2f_%.2f_%i_%i_%.2f_eq"%(save_adress,name_exo,np.amin(x_ratio_species_active),np.amax(x_ratio_species_active),T_iso,beta,P_tau/(1.e+5))
if Noise == True :
...
if Profil == True :
    ...
    if TopPressure == True :
        x_comp_Ref = np.amax(x_ratio_species_active)
        alp = R_gp*T_iso/(g0*M)*np.log(P_h/P_surf)
    ...
    x_comp_min, x_comp_max = np.amin(x_ratio_species_active), np.amax(x_ratio_species_active)
    d_lim = (Rp+h)*np.cos(np.pi/2.-beta_rad)
    alp_max = R_gp*T_iso/(g0*M)*np.log(P_tau/P_surf)
        ...
        for i_long in range(reso_long+1) :
            ...
            if x <= d_lim and i_lat != 0  and i_lat != reso_lat and beta_rad >= theta_step :
                if i_long >= 0. and i_long < reso_long/4. :
                    x_comp = x_comp_min + (d_lim - x)*(x_comp_max-x_comp_min)/(2*d_lim)
                if i_long >= 3*reso_long/2. and i_long < reso_long :
                    x_comp = x_comp_min + (d_lim - x)*(x_comp_max-x_comp_min)/(2*d_lim)
                if i_long >= reso_long/4. and i_long < reso_long/2. :
                    x_comp = x_comp_max - (d_lim - x)*(x_comp_max-x_comp_min)/(2*d_lim)
            ...
            if i_lat == 0 or i_lat == reso_lat :
                if beta_rad >= theta_step :
                    x_comp = (x_comp_max+x_comp_min)/2.
                else :
                    if i_long >= reso_long/4. and i_long < 3.*reso_long/4. :
                        x_comp = x_comp_max
                    else :
            ...
            for i_n in range(n_layers+2) :
                ...
                if z < n_lim :
                    data_convert[4,0,i_n,i_lat,i_long] = x_comp_max
                    data_convert[1,0,i_n,i_lat,i_long] = T_iso
                else :
                    if z_maxi == 0 :
                        z_maxi = z - delta_z
                        P_top = data_convert[0,0,i_n-1,i_lat,i_long]
                    data_convert[4,0,i_n,i_lat,i_long] = x_comp
                    data_convert[1,0,i_n,i_lat,i_long] = T_iso

                if Composition == False :
                    data_convert[4,0,i_n,i_lat,i_long] = x_comp
                    data_convert[2,0,i_n,i_lat,i_long] = (1-x_comp)/(1.+ratio_HeH2)
                    data_convert[3,0,i_n,i_lat,i_long] = data_convert[2,0,i_n,i_lat,i_long]*ratio_HeH2
                    M = np.nansum(M_species*data_convert[2:2+n_species.size,0,i_n,i_lat,i_long])
                    if z < n_lim :
                        data_convert[0,0,i_n,i_lat,i_long] = P_surf*np.exp(-g0*M/(R_gp*T_iso)*z/(1+z/Rp))
                    else :
